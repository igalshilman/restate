...
without adding feature-specific transport paths. Pull-driven remote cursors avoid work
ahead of downstream demand and stop promptly when the query is cancelled. Partial
...
pre-v1.8 binary; unknown peer versions remain strict. A worker can decline an incompatible
fragment before execution, in which case the querying node executes the same fragment
locally over the raw pull stream. A query that races with partition movement can likewise
return a transient error and should be retried.
