...
/// Cloneable control-plane handle retained by the server map. Its explicit
/// cancellation signal wakes the scanner task even while that task is polling
/// a pipeline-breaking fragment rather than waiting for another request.
#[derive(Clone)]
...
/// The result of polling a scanner stream while it remains live.
enum NextBatchPoll {
    Shutdown,
    Stream(Option<DataFusionResult<RecordBatch>>),
}

/// Polls one batch without making cancellation wait for a pipeline-breaking
/// fragment to produce output. Shutdown wins when both futures are ready.
