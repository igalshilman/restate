...
pub struct RemoteScannerManager {
    ...
    partition_routing: Option<PartitionRouting>,
...
impl RemoteScannerManager {
    pub fn new(
        ...
        Self {
            remote_scanner,
            partition_routing: Some(partition_routing),
    ...
    pub fn local_only(metadata: Metadata) -> Self {
        Self {
            remote_scanner: Arc::new(NoopRemoteScanner),
            partition_routing: None,
            local_scanners: LocalPartitionScannerRegistry::default(),
            metadata,
        }
    }
    ...
    fn get_partition_target_node(
        ...
        let Some(partition_routing) = &self.partition_routing else {
            return Ok(PartitionLocation::Local);
        };
        let my_node_id = self.metadata.my_node_id();
        match partition_routing.get_node_by_partition(partition_id) {
            None => bail!("node lookup for partition {partition_id} failed"),
            Some(node_id) if node_id == my_node_id => Ok(PartitionLocation::Local),
            Some(node_id) => Ok(PartitionLocation::Remote(NodeId::from(node_id))),
        }
    }
