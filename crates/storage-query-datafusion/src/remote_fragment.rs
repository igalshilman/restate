...
//! Serialization and execution for physical-plan fragments that run at a
//! partition owner.
...
impl RemoteFragment {
    ...
    /// Executes this immutable template over one pull-driven input stream.
...
/// Per-query execution-plan counters shared with the remote cursor clone that
/// resolves fragment negotiation.
#[derive(Clone)]
...
mod stream_input {
    ...
    /// Binds a fragment to a pull-driven stream and executes its sole output
    /// partition. The input stream is consumed exactly once by DataFusion.
