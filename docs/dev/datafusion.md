...
| `RemoteNodeExec` | `storage-query-datafusion/src/table_providers.rs` | Pulls one node's planned remote scan, optionally with a validated physical fragment |
...
When the partition is remote, the pull-driven remote cursor manages the RPC protocol:
...
scan. The full contract is documented in
